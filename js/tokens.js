...
if (typeof app !== 'undefined') {
	app.directive('bill', function () {
		return {
			...
			+'<span class="series">{{ngModel.pub}}</span><br><span class="wert"><span ng-if="ngModel.amount > 0">{{ngModel.amount|money}}</span><span ng-if="!(ngModel.amount > 0)" class="invalid">unbekannt / ungültig</span></span><br>{{currency.bank}}<br><a ng-href="currency.bankurl" target="_blank">{{currency.bankurl}}</a>'
